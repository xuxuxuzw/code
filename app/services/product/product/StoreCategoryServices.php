...
class StoreCategoryServices extends BaseServices
{
    ...
    {
        if (isset($info['pid'])) {
            $f[] = Form::select('pid', '父级', (string)($info['pid'] ?? ''))->setOptions($this->menus())->filterable(1)->required()->disabled(true);
        } else {
            $f[] = Form::select('pid', '父级', (string)($info['pid'] ?? ''))->setOptions($this->menus())->filterable(1)->required();
        }
        $f[] = Form::input('cate_name', '分类名称', $info['cate_name'] ?? '')->maxlength(30)->required();
        $f[] = Form::frameImageOne('pic', '分类图标(180*180)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'pic')), $info['pic'] ?? '')->icon('ios-add')->width('60%')->height('435px');
        $f[] = Form::number('sort', '排序', $info['sort'] ?? 0)->min(0);

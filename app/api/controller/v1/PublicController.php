...
class PublicController
{
    ...
    public function menu_user(Request $request)
    {
        ...
        foreach ($menusInfo as $key => &$value) {
            ...
            if ($value['id'] == 135 && !$vipOpen)
                unset($menusInfo[$key]);
            if ($value['id'] == 137 && (!sys_config('brokerage_func_status') || !($user->checkUserPromoter($uid, $userInfo))))
                unset($menusInfo[$key]);
            if ($value['id'] == 138 && !sys_config('balance_func_status'))
                unset($menusInfo[$key]);
            if ($value['id'] == 174 && !$userService)
                unset($menusInfo[$key]);
            if (((!$userService) && (!$systemStoreStaff->verifyStatus($uid))) && $value['url'] === '/pages/admin/order_cancellation/index')
                unset($menusInfo[$key]);
    ...
    public function pink(StorePinkServices $pink, UserServices $user)
    {
        ...
        $uids = $pink->getColumn(['status' => 2, 'is_refund' => 0], 'uid');
